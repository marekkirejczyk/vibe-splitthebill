...
The existing e2e drives the mock-fixture path via `Platform.OS === "web"`. After M6:

- The web build will also persist via `@react-native-async-storage/async-storage`'s localStorage backend. Each Playwright test spawns a fresh context (verified in `playwright.config.ts` — no `storageState`), so localStorage starts empty per run. No e2e cleanup needed.
- Add **two new test legs** to `apps/mobile/e2e/smoke.spec.ts` after the existing swipe leg:
...
```
pnpm -r lint
pnpm -r test                                              # core (59) + web (9) + mobile (M5 baseline 41 + ~10 new = ~51)
pnpm --filter web build
pnpm --filter @splitbill/mobile e2e                       # Playwright phase machine + 2 new legs
...
- `apps/mobile/src/components/SwipeableRow.test.tsx` (edit — 6 new edit cases on top of M5's 6 gesture cases)
...
| SwipeableRow edit tests | `pnpm --filter @splitbill/mobile test SwipeableRow` | M5's 6 gesture cases still pass + 6 new edit cases (open, blur-commit, blur-discard, price decimal-pad, comma parsing, invalid input). |
...
| All mobile tests | `pnpm --filter @splitbill/mobile test` | ~51 total; ~10 net new. |
