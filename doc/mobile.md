...
- `extra.apiSecret` — read from `process.env.EXPO_PUBLIC_API_SECRET` (M8), surfaced to the app by `src/lib/apiSecret.ts` and sent as the `x-splitbill-key` header on the `/api/extract` POST. Set per build via an **EAS secret** (`eas secret:create --name EXPO_PUBLIC_API_SECRET`), never committed; unset in dev, where the server gate is also off.
- `runtimeVersion: { policy: "appVersion" }` (M8) — ties OTA-update compatibility to the app version. EAS Update isn't wired yet, but setting the policy now avoids a config migration later.

`apps/mobile/eas.json` (M8): `appVersionSource: "remote"` (EAS owns build numbers / versionCodes — don't also hand-edit `buildNumber` / `versionCode`) with three build profiles — `development` (dev client), `preview` (internal-distribution `.ipa`/`.apk` for quick device smoke), `production` (store `.ipa`/`.aab`, `autoIncrement`) — and a `production` submit config targeting the Play `internal` track. Signing credentials are EAS-managed; `play-service-account.json` / `*.p8` / `*.keystore` are git-ignored.
...
| **M8** | EAS Build preview, TestFlight internal, Android internal track, physical-device smoke + shared-secret auth on `/api/extract` | 🚧 in progress — shared-secret gate + `eas.json` + config landed; `eas build`/`submit` + device installs are maintainer-run (paid Apple/Google/EAS accounts, network). See [`../plan-m8.md`](../plan-m8.md). |
...
- **Backend exposure — ✅ gated in M8 (server config pending).** `/api/extract` now runs an `isAuthorized` check: when `API_SHARED_SECRET` is set server-side, same-origin browser calls (the web app) are admitted by `Origin` match against `NEXT_PUBLIC_SITE_ORIGIN` — so no secret leaks into the public web bundle — and every other caller (the mobile binary, curl) must present a matching `x-splitbill-key` (constant-time compared). The gate stays open while the env var is unset, preserving local dev. Mobile sends the header from `extra.apiSecret`. **Remaining:** the maintainer must set `API_SHARED_SECRET` (+ `NEXT_PUBLIC_SITE_ORIGIN`) on Vercel and create the matching `EXPO_PUBLIC_API_SECRET` EAS secret. **Residual limitation:** a secret baked into a shipped binary is recoverable by reverse engineering — this stops drive-by abuse, not a determined attacker; App Attest / Play Integrity is the real fix and is deferred. Rotating the secret invalidates installed builds until a new one ships.
